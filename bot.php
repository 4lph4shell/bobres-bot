...
if ($data == "inviteSetting" && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    $stmt->execute();
    $inviteAmount = number_format($stmt->get_result()->fetch_assoc()['value'] ?? 0) . " تومان (SBT=Tomn)";
    $stmt->close();
    setUser();
    $keys = json_encode(['inline_keyboard' => [
        ...
        [
            ...
            ['text' => "مقدار پورسانت", 'callback_data' => "Alph4ir"]
        ],
        [
...
if ($data == "editInviteAmount" && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    delMessage();
    sendMessage("لطفا مبلغ پورسانت رو به تومان (SBT=Tomn) وارد کن", $cancelKey);
    setUser($data);
}
if ($userInfo['step'] == "editInviteAmount" && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if (is_numeric($text)) {
        ...
        $keys = json_encode(['inline_keyboard' => [
            [['text' => "❗️بنر دعوت", 'callback_data' => "inviteBanner"]],
            [
                ['text' => number_format($text) . " تومان (SBT=Tomn)", 'callback_data' => "editInviteAmount"],
                ['text' => "مقدار پورسانت", 'callback_data' => "Alph4ir"]
            ],
            [
                ['text' => $buttonValues['back_button'], 'callback_data' => "botSettings"]
...
if ($data == "myInfo") {
    ...
    $myWallet = number_format($userInfo['wallet']) . " تومان (SBT=Tomn)";
    ...
    if (is_numeric($text)) {
        if ($text > 0) {
            if ($userInfo['wallet'] >= $text) {
                ...
                sendMessage("✅|مبلغ " . number_format($text) . " تومان (SBT=Tomn) به کیف پول شما توسط کاربر $from_id انتقال یافت", null, null, $match[1]);
                setUser();
                sendMessage("✅|مبلغ " . number_format($text) . " تومان (SBT=Tomn) به کیف پول کاربر مورد نظر شما انتقال یافت", $removeKeyboard);
...
if ($data == "increaseMyWallet") {
    delMessage();
    sendMessage("🙂 عزیزم مقدار شارژ مورد نظر خود را به تومان (SBT=Tomn) وارد کن (بیشتر از 5000 تومان (SBT=Tomn))", $cancelKey);
    setUser($data);
}
...
if (preg_match('/^approvePayment(.*)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    sendMessage("افزایش حساب شما با موفقیت تأیید شد\n✅ مبلغ " . number_format($price) . " تومان (SBT=Tomn) به حساب شما اضافه شد", null, null, $userId);
...
if (preg_match('/^increaseWalletUser(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if (is_numeric($text)) {
        ...
        sendMessage("✅ مبلغ " . number_format($text) . " تومان (SBT=Tomn) به حساب شما اضافه شد", null, null, $match[1]);
        sendMessage("✅ مبلغ " . number_format($text) . " تومان (SBT=Tomn) به کیف پول کاربر مورد نظر اضافه شد", $removeKeyboard);
        sendMessage($mainValues['reached_main_menu'], getMainKeys());
        setUser();
    } else {
        sendMessage($mainValues['send_only_number']);
    ...
    for ($i = 1; $i <= $text; $i++) {
        ...
        🔮 $remark \n " . ($botState['configLinkState'] != "off" && $serverType != "marzban" ? "<code>$link</code>" : "");
            if ($botState['subLinkState'] == "on") $acc_text .=
                " \n🌐 (لینک خدمات سابسکرایبشن  (جهت استفاده از خدمات لطفا به قسمت آموزشی باشگاه مشتریان استرابری مراجعه نمایید : <code>$subLink</code>";
    ...
    if ($payInfo['state'] == "pending") {
        ...
        if ($request_json->payment_status == 'finished' or $request_json->payment_status == 'confirmed' or $request_json->payment_status == 'sending') {
            ...
            if ($payType == "INCREASE_WALLET") {
                ...
                sendMessage("افزایش حساب شما با موفقیت تأیید شد\n✅ مبلغ " . number_format($price) . " تومان (SBT=Tomn) به حساب شما اضافه شد");
                sendMessage("✅ مبلغ " . number_format($price) . " تومان (SBT=Tomn) به کیف پول کاربر $from_id توسط درگاه ارزی ریالی اضافه شد", null, null, $admin);
...
" . ($botState['configLinkState'] != "off" && $serverType != "marzban" ? "
 🔗 لینک دسترسی مستقیم کانفیگ شما ( لینک کاربری شما برای استفاده در برنامه های مجری و خدمات استرابری سرور) :
  
 <code>$link</code>" : "");

                        if ($botState['subLinkState'] == "on") $acc_text .= "

🔋 لینک نمایش اطلاعات حجم و مدت زمان باقی مانده کاربری شما (برای بهره مندی لینک را کپی کرده و در مروگر خود آدرس مورد نظر اجرا نمایید):
 
<code> $botUrl" . "search.php?id=" . $uniqid . "</code>


🌐 لینک خدمات سابسکرایبشن (جهت بهره مندی از این خدمات وارد بخش آموزشی باشگاه مشتریان استرابری سرور مراجعه نمایید):
        ...
        ";
            ...
            VALUES (?, ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?,1, ?, 0, ?, ?);");
                ...
                if ($userInfo['refered_by'] != null) {
                    ...
                    sendMessage("تبریک یکی از زیر مجموعه های شما خرید انجام داد شما مبلغ " . number_format($inviteAmount) . " تومان (SBT=Tomn) جایزه دریافت کردید", null, null, $inviterId);
                }
                $keys = json_encode(['inline_keyboard' => [
                    [
                        ['text' => "بنازم خرید جدید ❤️", 'callback_data' => "Alph4ir"]
                    ],
                ]]);
                ...
                $keys = json_encode(['inline_keyboard' => [
                    [
                        ['text' => "به به تمدید 😍", 'callback_data' => "Alph4ir"]
                    ],
                ]]);
                ...
                if ($response->success) {
                    ...
                    $keys = json_encode(['inline_keyboard' => [
                        [
                            ['text' => "اخیش یکی زمان زد 😁", 'callback_data' => "Alph4ir"]
                        ],
                    ]]);
                    sendMessage("
...
", $keys, "html", $admin);
                ...
                if ($response->success) {
                    ...
                    $keys = json_encode(['inline_keyboard' => [
                        [
                            ['text' => "اخیش یکی حجم زد 😁", 'callback_data' => "Alph4ir"]
                        ],
                    ]]);
                    sendMessage("
...
", $keys, "html", $admin);
    ...
    ", $keys, "html", $admin);
            ...
            editKeys(json_encode(['inline_keyboard' => [
                [['text' => "پرداخت انجام شد", 'callback_data' => "Alph4ir"]]
            ]]));
        } else {
...
if (preg_match('/selectCategory(?<categoryId>\d+)_(?<serverId>\d+)_(?<buyType>\w+)/', $data, $match) && ($botState['sellState'] == "on" || $from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($respd->num_rows == 0) {
        ...
        while ($file = $respd->fetch_assoc()) {
            ...
            $price = ($price == 0) ? 'رایگان' : number_format($price) . ' تومان (SBT=Tomn) ';
        ...
        if ($list->num_rows > 0) {
            ...
            if ($canUse > $userUsedCount) {
                ...
                if ($type == "percent") {
                    ...
                    $price -= $discount;
                    $discount = number_format($discount) . " تومان (SBT=Tomn)";
                } else {
                    $price -= $amount;
                    $discount = number_format($amount) . " تومان (SBT=Tomn)";
                }
                if ($price < 0) $price = 0;
...
if ((preg_match('/^discountSelectPlan(\d+)_(\d+)_(\d+)/', $userInfo['step'], $match) ||
    ...
    $priceC = ($price == 0) ? 'رایگان' : number_format($price) . ' تومان (SBT=Tomn) ';
    if (isset($accountCount)) {
        $eachPrice = number_format($price / $accountCount) . " تومان (SBT=Tomn)";
...
<code>$subLink</code>";
    ...
    unset($markup[count($markup) - 1]);
    $markup[] = [['text' => "✅", 'callback_data' => "Alph4ir"]];
    $keys = json_encode(['inline_keyboard' => array_values($markup)], 488);
    ...
    if ($admin != $from_id) {
        $keys = json_encode(['inline_keyboard' => [
            [
                ['text' => "به به 🛍", 'callback_data' => "Alph4ir"]
            ],
        ]]);
        $msg = str_replace(
...
if ($data == "availableServers") {
    ...
    $keys = array();
    $keys[] = [
        ['text' => "تعداد باقیمانده", 'callback_data' => "Alph4ir"],
        ['text' => "پلن", 'callback_data' => "Alph4ir"],
        ['text' => 'سرور', 'callback_data' => "Alph4ir"]
    ];
    while ($file_detail = $serversList->fetch_assoc()) {
        ...
        if ($name->num_rows > 0) {
            ...
            $keys[] = [
                ['text' => $acount . " اکانت", 'callback_data' => "Alph4ir"],
...
if ($data == "availableServers2") {
    ...
    while ($file_detail2 = $serversList->fetch_assoc()) {
        ...
        if ($name->num_rows > 0) {
            ...
            $keys[] = [
                ['text' => $acount2 . " اکانت", 'callback_data' => "Alph4ir"],
                ['text' => $title2 ?? " ", 'callback_data' => "Alph4ir"],
            ];
        }
    }
    $keys[] = [['text' => $buttonValues['back_button'], 'callback_data' => "mainMenu"]];
...
if (preg_match('/^agencyDecline(\d+)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    editKeys(json_encode(['inline_keyboard' => [
        [['text' => $buttonValues['declined'], 'callback_data' => "Alph4ir"]]
    ]]));
...
if (preg_match('/^agencyApprove(\d+)_(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    if (is_numeric($text)) {
        editKeys(json_encode(['inline_keyboard' => [
            [['text' => $buttonValues['approved'], 'callback_data' => "Alph4ir"]]
        ]]), $match[2]);
...
if (preg_match('/decline(\d+)_(\d+)/', $userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) and $text != $buttonValues['cancel']) {
    ...
    editKeys(
        json_encode(['inline_keyboard' => [
            [['text' => "لغو شد ❌", 'callback_data' => "Alph4ir"]]
        ]]),
        $match[2]
    );
...
if ($data == 'dayPlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    $keyboard = [];
    $keyboard[] = [['text' => "حذف", 'callback_data' => "Alph4ir"], ['text' => "قیمت", 'callback_data' => "Alph4ir"], ['text' => "تعداد روز", 'callback_data' => "Alph4ir"]];
    while ($cat = $res->fetch_assoc()) {
        $id = $cat['id'];
        $title = $cat['volume'];
        $price = number_format($cat['price']) . " تومان (SBT=Tomn)";
        $acount = $cat['acount'];
...
مقدار دوم قیمت (30000) تومان (SBT=Tomn)
 ", $cancelKey);
    exit;
}
...
if ($data == 'volumePlanSettings' and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    $keyboard = [];
    $keyboard[] = [['text' => "حذف", 'callback_data' => "Alph4ir"], ['text' => "قیمت", 'callback_data' => "Alph4ir"], ['text' => "مقدار حجم", 'callback_data' => "Alph4ir"]];
...
if (preg_match('/^closeTicket_(\d+)/', $data, $match) and  $from_id != $admin) {
    ...
    $keys = json_encode(['inline_keyboard' => [
        [
            ['text' => "$from_id", 'callback_data' => "Alph4ir"],
            ['text' => "آیدی کاربر", 'callback_data' => 'Alph4ir']
        ],
        [
            ['text' => $first_name ?? " ", 'callback_data' => "Alph4ir"],
            ['text' => "اسم کاربر", 'callback_data' => 'Alph4ir']
        ],
        [
            ['text' => "$title", 'callback_data' => 'Alph4ir'],
            ['text' => "عنوان", 'callback_data' => 'Alph4ir']
        ],
        [
            ['text' => "$category", 'callback_data' => 'Alph4ir'],
            ['text' => "دسته بندی", 'callback_data' => 'Alph4ir']
        ],
    ]]);
...
if (preg_match("/^rate_+([0-9])+_+([0-9])/", $data, $match)) {
    ...
    $keys = json_encode(['inline_keyboard' => [
        [
            ['text' => "رای تیکت", 'callback_data' => "Alph4ir"]
        ],
    ]]);
...
if ($data == 'ticketsCategory' and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    $stmt->close();
    $keys = array();
    $keys[] = [['text' => "حذف", 'callback_data' => "Alph4ir"], ['text' => "دسته بندی", 'callback_data' => "Alph4ir"]];

    if ($ticketCategory->num_rows > 0) {
        while ($row = $ticketCategory->fetch_assoc()) {
            $rowId = $row['id'];
            $ticketName = $row['value'];
            $keys[] = [['text' => "❌", 'callback_data' => "delTicketCat_$rowId"], ['text' => $ticketName, 'callback_data' => "Alph4ir"]];
        }
    } else {
...
if (preg_match('/^closeTicket_(\d+)/', $data, $match) and  ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    editKeys(json_encode(['inline_keyboard' => [
        [['text' => "تیکت بسته شد", 'callback_data' => "Alph4ir"]]
    ]]));
}
...
if ($userInfo['step'] == "showAccount" and $text != $buttonValues['cancel']) {
    ...
    if (!$found) {
        ...
        $keys = json_encode(['inline_keyboard' => array_merge([
            [
                ['text' => $state ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "🔘 وضعیت اکانت 🔘", 'callback_data' => "Alph4ir"],
            ],
            [
                ['text' => $remark ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "« نام اکانت »", 'callback_data' => "Alph4ir"],
            ]
        ], (!$isMarzban ? [
            [
                ['text' => $upload ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "√ آپلود √", 'callback_data' => "Alph4ir"],
            ],
            [
                ['text' => $download ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "√ دانلود √", 'callback_data' => "Alph4ir"],
            ]
        ] : [
            [
                ['text' => $totalUsed ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "√ آپلود + دانلود √", 'callback_data' => "Alph4ir"],
            ]
        ]), [
            [
                ['text' => $total ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "† حجم کلی †", 'callback_data' => "Alph4ir"],
            ],
            [
                ['text' => $leftMb ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "~ حجم باقیمانده ~", 'callback_data' => "Alph4ir"],
            ],
            [
                ['text' => $expiryTime ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "تاریخ اتمام", 'callback_data' => "Alph4ir"],
            ],
            [
                ['text' => $expiryDay ?? " ", 'callback_data' => "Alph4ir"],
                ['text' => "تعداد روز باقیمانده", 'callback_data' => "Alph4ir"],
            ],
...
if (preg_match('/(addNewRahgozarPlan|addNewPlan|addNewMarzbanPlan)/', $userInfo['step']) and $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    if ($step == 1 and $text != $buttonValues['cancel']) {
        $msg = '🔰 لطفا قیمت پلن رو به تومان (SBT=Tomn) وارد کنید!';
        if (strlen($text) > 1) {
...
if ($data == 'backplan' and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    $keyboard[] = [
        ['text' => '➕ افزودن پلن بابی', 'callback_data' => "addNewRahgozarPlan"],
...
if (($data == "editCustomPlan" || preg_match('/^editCustom(gbPrice|dayPrice)/', $userInfo['step'], $match)) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    ...
    $gbPrice = number_format($botState['gbPrice'] ?? 0) . " تومان (SBT=Tomn)";
    $dayPrice = number_format($botState['dayPrice'] ?? 0) . " تومان (SBT=Tomn)";

    $keys = json_encode(['inline_keyboard' => [
        [
            ['text' => $gbPrice, 'callback_data' => "editCustomgbPrice"],
            ['text' => "هزینه هر گیگ", 'callback_data' => "Alph4ir"]
        ],
        [
            ['text' => $dayPrice, 'callback_data' => "editCustomdayPrice"],
            ['text' => "هزینه هر روز", 'callback_data' => "Alph4ir"]
        ],
        [
            ['text' => $buttonValues['back_button'], 'callback_data' => "backplan"]
        ]
...
if (preg_match('/^editCustom(gbPrice|dayPrice)/', $data, $match) && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    ...
    sendMessage("لطفا هزینه " . $title . " را به تومان (SBT=Tomn) وارد کنید", $cancelKey);
    setUser($data);
}
...
if (preg_match('/^bobresplanacclist(\d+)/', $data, $match) and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
...
if (preg_match('/^bobresplandelete(\d+)/', $data, $match) and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
...
if (preg_match('/^bobresplanname(\d+)/', $data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    setUser($data);
    delMessage();
    sendMessage("🔅 یه اسم برا پلن جدید انتخاب کن:", $cancelKey);
    exit;
}
if (preg_match('/^bobresplanname(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
...
if (preg_match('/^bobresplanslimit(\d+)/', $data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    setUser($data);
    delMessage();
    sendMessage("🔅 ظرفیت جدید برای پلن انتخاب کن:", $cancelKey);
    exit;
}
if (preg_match('/^bobresplanslimit(\d+)/', $userInfo['step'], $match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
...
if (preg_match('/^bobresplansinobundid(\d+)/', $data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    setUser($data);
    delMessage();
    sendMessage("🔅 سطر جدید برای پلن انتخاب کن:", $cancelKey);
    exit;
}
if (preg_match('/^bobresplansinobundid(\d+)/', $userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
...
if (preg_match('/^bobresplaneditdes(\d+)/', $data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    setUser($data);
    delMessage();
    sendMessage("🎯 توضیحاتت رو برام وارد کن:", $cancelKey);
    exit;
}
if (preg_match('/^bobresplaneditdes(\d+)/', $userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
...
if (preg_match('/^bobresplanrial(\d+)/', $data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    setUser($data);
    delMessage();
    sendMessage("🎯 شیطون قیمت و گرون کردی 😂 ، خب قیمت جدید و بزن ببینم :", $cancelKey);
    exit;
}
if (preg_match('/^bobresplanrial(\d+)/', $userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    if (is_numeric($text)) {
...
if (preg_match('/payRenewWithWallet(.*)/', $data, $match)) {
    ...
    if ($userwallet < $price) {
        ...
        alert("💡موجودی کیف پول (" . number_format($userwallet) . " تومان (SBT=Tomn)) کافی نیست لطفا به مقدار " . number_format($needamount) . " تومان (SBT=Tomn) شارژ کنید ", true);
        exit;
    }
...
if (preg_match('/increaseADay(.*)/', $data, $match)) {
    ...
    while ($cat = $res->fetch_assoc()) {
        ...
        if ($price == 0) $price = "رایگان";
        else $price = number_format($price) . " تومان (SBT=Tomn)";
...
if (preg_match('/increaseVolumePlan(?<orderId>.+)_(?<volumeId>.+)/', $data, $match)) {
    ...
    else $planprice = " " . number_format($planprice) . " تومان (SBT=Tomn)";
...
if (preg_match('/^bobrescategorydelete(\d+)_(\d+)/', $data, $match) and ($from_id == $admin || $userInfo['isAdmin'] == true)) {
...
if (preg_match('/^bobrescategoryedit/', $data) and ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
    setUser($data);
    delMessage();
    sendMessage("〽️ یه اسم جدید برا دسته بندی انتخاب کن:", $cancelKey);
    exit;
}
if (preg_match('/bobrescategoryedit(\d+)_(\d+)/', $userInfo['step'], $match) && ($from_id == $admin || $userInfo['isAdmin'] == true) && $text != $buttonValues['cancel']) {
...
if (preg_match('/^bobresdeleteserver(\d+)/', $data, $match) and ($from_id == $admin || ($userInfo['isAdmin'] == true && $permissions['servers']))) {
...
if ($data == "addDiscountCode" && ($from_id == $admin || $userInfo['isAdmin'] == true)) {
    delMessage();
    sendMessage("🔘|لطفا مقدار تخفیف را وارد کنید\nبرای درصد علامت % را در کنار عدد وارد کنید در غیر آن مقدار تخفیف به تومان (SBT=Tomn) محاسبه میشود", $cancelKey);
    setUser($data);
}
