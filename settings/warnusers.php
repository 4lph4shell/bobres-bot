    ...
    if($orders->num_rows>0){
        while ($order = $orders->fetch_assoc()){
            ...
            if($expiryTime != null && $total != null && $expiryTime >= 0 && $notif == 0){
                ...
                if($send != ""){  
                    $msg = "💡 کاربر گرامی استرابری سرور، 
        از سرویس اشتراک $remark تنها (۱ $send) باقی مانده است. میتوانید از قسمت کانفیگ های خریداری شده، سرویس فعلی خود را تمدید کنید یا سرویس جدید خریداری کنید.";
    ...
    if($orders->num_rows>0){
        while ($order = $orders->fetch_assoc()){
            ...
            if($send){
        		...
        		if(!is_null($res)){
                    $msg = "💡 کاربر گرامی استرابری سرور،
    اشتراک سرویس $remark منقضی شد و از لیست سفارش و کانفیگ ها حذف گردید. لطفا از بخش فروشگاه استرابری, سرویس و اشتراک جدید خریداری کنید.";
                    sendMessage( $msg, null, null, $from_id);
